...
p2 <- ggplot(aes(V1, V2, fill = V1),data = df) + geom_violin() 
p2 <- p2 + theme(axis.text.x=element_blank()) + guides(fill=FALSE) 
p2 <- p2 + ylab("") + xlab("") + ggtitle("Violin")

p3 <- sinaplot(df[,2], df[,1], size = 1.5) + ggtitle("Sina")
p3 <- p3+ theme(axis.text.x = element_text(angle = 0, hjust = 0.5)) 

beeswarm2 <- beeswarm(df[,2] ~ df[,1])
p4 <- ggplot(beeswarm2, aes(x, y)) + geom_point(aes(colour = df$V1)) + scale_x_discrete(limits = labels)
p4 <- p4 + theme(axis.text.x = element_text(angle = 0, hjust = 0.5)) + guides(colour=FALSE) 
p4 <- p4 + ylab("log2 expression") + xlab("") + ggtitle("Bee Swarm")
